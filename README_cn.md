...
*   **Wi-Fi 設定:** 透過 BLE 直接設定 ESP32 的 Wi-Fi 連線資訊 (SSID 與密碼)，介面現已全面升級，支援 Material Design 風格與輸入驗證。
...
*   **統一命名規範:** 所有的 APK 檔案名稱與版本號現在嚴格遵循 `X.Y.Z-channel-count` 格式 (例如 `1.2.1-dev-255`)，移除所有空格與特殊字元，確保在不同環境下的行為一致。
